
<section class="text-center">

    <div class="payment-area text-center">

        <div class="payment-method">
            <img class="payment-method-img" src="<?php echo $this->dados_globais['caminho_externo_img'] . "banks/nubank.png";?>" >
            <label class="payment-method-text">NuConta - 260</label>
            <label class="payment-method-text">Titular: Lucas Bione</label>
            <label class="payment-method-text">CPF: 082.527.624-17</label>
            <label class="payment-method-text">Agência: 0001</label>
            <label class="payment-method-text">Conta: 79622950-5</label>
            <label class="payment-method-text">Tipo: NuConta</label>
        </div>
        <div class="payment-method">
            <img class="payment-method-img" src="<?php echo $this->dados_globais['caminho_externo_img'] . "banks/inter.png";?>" >
            <label class="payment-method-text">Inter - 077</label>
            <label class="payment-method-text">Titular: Lucas Bione</label>
            <label class="payment-method-text">CPF: 082.527.624-17</label>
            <label class="payment-method-text">Agência: 0001</label>
            <label class="payment-method-text">Conta: 38049635</label>
            <label class="payment-method-text">&nbsp;</label>
        </div>
        <div class="payment-method">
            <img class="payment-method-img" src="<?php echo $this->dados_globais['caminho_externo_img'] . "banks/bancodobrasil.png";?>" >
            <label class="payment-method-text">Banco do Brasil - 001</label>
            <label class="payment-method-text">Titular: Lucas Bione</label>
            <label class="payment-method-text">CPF: 082.527.624-17</label>
            <label class="payment-method-text">Agência: 1228-2</label>
            <label class="payment-method-text">Conta: 111025-0</label>
            <label class="payment-method-text">Tipo: Conta Corrente</label>
        </div>
        <div class="payment-method">
            <img class="payment-method-img" src="<?php echo $this->dados_globais['caminho_externo_img'] . "banks/picpay.png";?>" >
            <label class="payment-method-text"><a href="https://picpay.me/rifasdosertao">@rifasdosertao</a></label>
            <label class="payment-method-text">Titular: Lucas Bione</label>
            <label class="payment-method-text">&nbsp;</label>
            <label class="payment-method-text">&nbsp;</label>
            <label class="payment-method-text">&nbsp;</label>
            <label class="payment-method-text">&nbsp;</label>
        </div>

    </div>
</section>